              ...
              <ul class="sui-nav">
                <li :class="{ active: isone }" @click="changeOrder('1')">
                  <a href="javascript:;">
                    ...
                    <span v-else-if="isdesc && isone">⬇</span>
                  </a>
                </li>
                <li :class="{ active: istwo }" @click="changeOrder('2')">
                  <a href="javascript:">
                    ...
                    <span v-else-if="istwo && isdesc">⬇</span>
                  </a>
                </li>
              </ul>
            </div>
...
export default {
  ...
  mounted() {
    this.getData();
    //全局事件总线，获取子组件传递trademark数据
    this.$bus.$on("getTrademark", (trademark) => {
      this.searchParams.trademark = `${trademark.tmId}:${trademark.tmName}`;
      this.getData();
    });
    //获取子组件传递props数据
    this.$bus.$on("getAttrInfo", (attr, attrValue) => {
  ...
  methods: {
    ...
    //将id赋值为undefined，发送请求时不会一起发送，有利于性能优化
    changeCategoryId() {
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
    },
    //移除categoryName三级列表面包屑
    removeCategoryName() {
