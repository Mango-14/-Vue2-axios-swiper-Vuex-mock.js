...
export default {
  ...
  mounted() {
    this.$store.dispatch("detail/detailList", this.$route.params.skuId);
  },
  computed: {
    ...
    spuSaleAttrValueListColor() {
      return this.spuSaleAttrList[0].spuSaleAttrValueList || [];
    },
    //商品选择的颜色
    color() {
      return (
        this.spuSaleAttrValueListColor.filter((item) => {
          return item.isChecked == 1;
        })[0].saleAttrValueName || ""
      );
    },
    //商品选择的版本
    spuSaleAttrValueListVersion() {
      return this.spuSaleAttrList[1].spuSaleAttrValueList || [];
    },
    version() {
      return (
        this.spuSaleAttrValueListVersion.filter((item) => {
          return item.isChecked == 1;
        })[0].saleAttrValueName || ""
      );
    },
  ...
  methods: {
    ...
    verifySkuNum() {
      if (this.skuNum == "") {
        ...
        this.skuNum = this.skuNum.replace(/^0*|[^\d]/g, "");
      }
    },
    /* async addShopToCart() {
      try {
        await this.$store.dispatch("detail/addShopToCart", {
    ...
    async addShopToCart() {
      try {
        await this.$store.dispatch("detail/addShopToCart", {
          //userId: localStorage.getItem("NANOIDTOKEN") || "",
          shopData: [
            {
              id: this.skuInfo.id,
              skuNum: this.skuNum,
